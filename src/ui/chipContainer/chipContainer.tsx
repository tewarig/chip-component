...
function ChipContainer(props: IChipContainerProps) {
  ...
  const [selectedFilteredOptionIndex, setSelectedFilteredOptionIndex] =
    useState<number>(0);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (
      e.key === "ArrowDown" &&
      selectedFilteredOptionIndex < filteredOptions.length - 1
    ) {
      setSelectedFilteredOptionIndex(selectedFilteredOptionIndex + 1);
      if (optionContainerRef.current) {
        const element = optionContainerRef.current;
    ...
    <div className="chip-container" style={{ width }} onClick={focusInput}>
      ...
      <div className="chip-input-container">
        ...
        <div
          ...
          ref={optionContainerRef as React.RefObject<HTMLDivElement>}
        >
          {showFilteredOptions &&
            filteredOptions.map((option, index) => (
              <button
                className={classNames("chip-autocomplete-options", {
                  "chip-autocomplete-options-selected":
                    index === selectedFilteredOptionIndex,
                })}
                key={index}
              ...
              >
                {option.avatar && (
                  <img
                    className="chip-autocomplete-avatar"
                    src={option.avatar}
                  />
                )}
